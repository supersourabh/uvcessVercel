...
export default function ProblemsScreen() {
    ...
    const submitHandler = (e) => {
        e.preventDefault()
        if(problem===null || about===null){
            alert("empty problem not accepted ...!!")
        }
        else{
        ...
        <div className="problems" >
            {
                loading ? <Loading /> :
                    error ? <Alerts info="danger" message={ error } /> :
                        success ?
                            <>
                                <Alerts info='success' message={ "Problem saved successfully, we will solve it soon... " } />
                                <div style={{backgroundColor : " #fcf0fc" , margin : 20}} className="problemsInfo">
